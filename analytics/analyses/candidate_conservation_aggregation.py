...
class CandidateAggregationSource:
    paths: tuple[Path, ...]
    columns: tuple[str, ...]
    row_count: int | None
    header: bool
...
class CandidateAlleleStore:
    ...
    def __init__(
        ...
        *,
        ...
        try:
            ...
            self.connection.execute(f"SET temp_directory={_sql_string(temp_dir)}")
    ...
    def _build_alleles(self) -> None:
        ...
        source_sql = _source_sql(self.source)
        self.connection.execute(
...
def resolve_candidate_aggregation_source(path: Path) -> CandidateAggregationSource:
    """Prefer validated, narrow VEP input partitions over an enriched merged table."""
    ...
    if path.name == "variant_annotations.vep.tsv.gz" and plan_path.exists():
        ...
        return CandidateAggregationSource(
            paths=tuple(paths),
            columns=columns,
            row_count=row_count,
            header=True,
...
def _source_sql(source: CandidateAggregationSource) -> str:
    columns = "{" + ",".join(
...
def _require_columns(columns: tuple[str, ...], path: Path) -> None:
    ...
    if missing:
        raise ValueError(
            f"Variant annotations {path} missing candidate columns: {', '.join(sorted(missing))}"
        )
